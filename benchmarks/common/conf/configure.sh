...
if [[ ( $GEN_AGGREGATION == "true" || $GEN_JOIN == "true" || $GEN_SCAN == "true" ) ]]; then
	...
	if [[ ! -d $HIVE_HOME ]]; then
		...
		TMP_HIVE_DIR=$BDEV_TOOLS_DIR/apache-hive-${HIVE_VERSION}-bin
		...
		if [[ $? != 0 ]]; then
			...
			TMP_HIVE_FILE=$BDEV_TOOLS_DIR/hive-${HIVE_VERSION}-bin.tar.gz
			TMP_HIVE_DIR=$BDEV_TOOLS_DIR/hive-${HIVE_VERSION}-bin
		...
		tar -xzf $TMP_HIVE_FILE -C $BDEV_TOOLS_DIR
