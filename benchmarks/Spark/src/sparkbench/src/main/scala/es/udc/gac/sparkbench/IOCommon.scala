...
class IOCommon() extends java.io.Serializable {
  ...
  def load_rdd(filename: String, sc: SparkContext, input_format: String = "Sequence") = {
  ...
  def save_rdd[T1, T2](filename: String, data: RDD[(T1, T2)], sc: SparkContext, output_format: String = "Sequence") = {
  ...
  def save_dataset[T1, T2](filename: String, dataset: Dataset[(T1, T2)], session: SparkSession, output_format: String = "Sequence") = {
      ...
      output_format match {
        ...
        case _ =>
          save_rdd(filename, dataset.rdd, session.sparkContext, output_format)
      }
  }
}
