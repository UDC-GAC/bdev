...
public class ConCmpt extends Configured implements Tool 
{
    ...
    {
		System.out.println("ConCmpt <input_path> <output_path> <# of nodes> <# of reducers> <max iteration> <makesym or nosym> <new or contNN>");
    ...
    public int run (final String[] args) throws Exception
    {
		...
		output_path = new Path(args[1]+"/concmpt_output");
		curbm_path = new Path(args[1]+"/concmpt_curbm");
		tempbm_path = new Path(args[1]+"/concmpt_tmp");
		nextbm_path = new Path(args[1]+"/concmpt_nextbm");
		//summaryout_path = new Path(args[1]+"/concmpt_summaryout");
		number_nodes = Integer.parseInt(args[2]);
		nreducers = Integer.parseInt(args[3]);
		niteration = Integer.parseInt(args[4]);

		if (niteration > MAX_ITERATIONS)
			niteration = MAX_ITERATIONS;;
		...
		if( args[6].compareTo("new") == 0 )
			start_from_newbm = 1;
		else {	// args[7] == contNN        e.g.) cont10
			start_from_newbm = 0;
			cur_iter = Integer.parseInt(args[6].substring(4));
			System.out.println("Starting from cur_iter = " + cur_iter);
		}

		System.out.println("\n-----===[PEGASUS: A Peta-Scale Graph Mining System]===-----\n");
		System.out.println("[PEGASUS] Computing Connected Component. Max iteration = " +niteration+ ", edge path = " +args[0]+ ", reducers = " +nreducers+ "\n");

		String bench_output_path = System.getenv("BENCHMARK_OUTPUT_DIR");
		local_output_path = bench_output_path + "/concmpt_output_temp";
		...
		for (i = cur_iter; i <= niteration; i++) {
			...
			System.out.println("Iteration = " + i + ", changed = " + ri.changed + ", unchanged = " + ri.unchanged);
		...
		System.out.println("[PEGASUS] Max iteration = " + niteration + ", iterations = " + iter_counter);
