...
export SPARK_KRYO_REGISTRATION_REQUIRED=false # If set to false, Kryo will write unregistered class names along with each object, which can cause performance overhead
...
export FLINK_TASKMANAGER_MEMORY_SEGMENT_SIZE="64kb" # Size of memory buffers used by the network stack and the memory manager (it requires a size-unit specifier)
export FLINK_TASKMANAGER_MEMORY_OFF_HEAP_SHUFFLE_SIZE="64mb" # Size of memory used by sort-merge blocking shuffle for shuffle data read (it requires a size-unit specifier)
export FLINK_TASKMANAGER_MEMORY_OFF_HEAP_SIZE="128mb" # Framework off-heap memory size for TaskManagers (it requires a size-unit specifier)
