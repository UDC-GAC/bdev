...
#export NODEMANAGER_MEMORY=`op_int "($MEMORY_PER_NODE * $NODEMANAGER_MEMORY_FACTOR) - $APP_MASTER_MEMORY"` # Memory available for allocation
#export NODEMANAGER_VCORES=$CORES_PER_NODE	# Number of cores per NodeManager
#export CONTAINER_MEMORY=`op_int "$NODEMANAGER_MEMORY / $NODEMANAGER_VCORES"`	# Memory available per container
