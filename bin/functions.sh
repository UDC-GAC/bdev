...
function generate_framework_config() {
    ...
    local target_lib_dir="$5"	# Lib folder in destination
    local master_file="${6:-}"	# Masters file path (optional)
    local workers_file="${7:-}"	# Workers file path (optional)
    
    [[ -z "$src_dir" || -z "$template_dir" || -z "$target_dir" || -z "$target_log_dir" || -z "$target_lib_dir" ]] && \
        m_exit "generate_framework_config: src_dir, template_dir, target_dir, target_log_dir and target_lib_dir are required"
    ...
    if ! mkdir -p "$target_lib_dir"; then
...
function set_framework() {
	...
	if [[ "$RESOURCE_MANAGER" == "yarn" ]]; then
		m_echo "Cluster resource manager: YARN"
	else
		m_echo "Cluster resource manager: ${RESOURCE_MANAGER^}"
	fi
