...
if [[ "${STORAGE_BACKEND,,}" == "hdfs" ]]; then
    (
        ...
        if [[ "$SECONDARY_NAMENODE" == "true" ]]; then
			#Secondary NameNode
            "$HADOOP_HOME/sbin/hadoop-daemons.sh" --config "$HADOOP_CONF_DIR" --script hdfs start secondarynamenode &
        fi
...
if [[ "$TIMELINE_SERVER" == "true" ]]; then
    (
        ...
        "$HADOOP_HOME/sbin/yarn-daemon.sh" --config "$HADOOP_CONF_DIR" start timelineserver
    ) &
fi
